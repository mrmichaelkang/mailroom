...
function Mailroom() {
  ...
  const getPackageList =  async () => {
    try {
      let res = await fetch(`http://localhost:5000/api/v1/${localStorage.getItem('uid')}/packages`);
  ...
  const handleAddPackage = newPackage => {
    fetch('http://localhost:5000/api/v1/add-package', {
      method: 'POST',
      cache: 'no-cache',
      headers: {
  ...
  const handleDelete = (event) => {
    ...
    fetch(`http://localhost:5000/api/v1/delete-package/${id}`, {
      method: "DELETE"
    })
  ...
  const handleUpdate = async (event) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/v1/update-package/${id}`, {
        method: "PUT"
      });
